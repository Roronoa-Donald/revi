...
const str = `    function buildMiniCourse(project, stepText) {
      if (project.pedagogy) {
...
code = code.replace(/    function buildMiniCourse[\s\S]*?function currentProgress\(\) \{/m, str);

fs.writeFileSync('preparation-web/assets/js/app.js', code);
console.log('Fixed renderLesson 4th time');
