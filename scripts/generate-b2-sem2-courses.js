...
const courses = [
  ...
  },
  {
    ...
    qTopics: [
      ['API', 'Une API definit une interface de communication entre programmes.', ['Une API est seulement une page HTML', 'Une API est toujours une image', 'Une API remplace le reseau'], 1],
      ['HTTP', 'HTTP transporte des requetes et reponses entre client et serveur.', ['HTTP est un langage de style', 'HTTP est une base SQL', 'HTTP est uniquement local'], 1],
      ['JSON', 'JSON est un format texte pour echanger des donnees structurees.', ['JSON compile Python', 'JSON est un navigateur', 'JSON stocke uniquement des images'], 1],
      ['REST', 'REST manipule des ressources avec les methodes HTTP.', ['REST ignore les routes', 'REST impose HTML', 'REST signifie base de donnees'], 2],
      ['GET', 'GET sert a recuperer une ressource.', ['GET supprime toujours', 'GET cree toujours', 'GET modifie le serveur par definition'], 2],
      ['POST', 'POST sert souvent a creer une ressource via une collection.', ['POST lit seulement', 'POST est interdit en API', 'POST remplace JSON'], 2],
      ['PUT', 'PUT cible une ressource identifiee pour la mettre a jour.', ['PUT ne prend jamais d id', 'PUT est identique a GET', 'PUT sert a lire uniquement'], 2],
      ['DELETE', 'DELETE supprime une ressource identifiee.', ['DELETE liste toutes les ressources', 'DELETE cree un utilisateur', 'DELETE remplace les headers'], 2],
      ['Flask', 'Flask permet de declarer des routes HTTP en Python.', ['Flask est un SGBD', 'Flask est un format JSON', 'Flask est un outil React'], 3],
      ['jsonify', 'jsonify renvoie une reponse JSON depuis Flask.', ['jsonify compile le CSS', 'jsonify cree un terminal', 'jsonify supprime les routes'], 3],
      ['request.get_json', 'request.get_json lit le body JSON envoye au serveur.', ['Il lit le CSS', 'Il lance Vite', 'Il remplace status code'], 3],
      ['200', '200 signifie OK.', ['200 signifie non authentifie', '200 signifie ressource introuvable', '200 signifie serveur casse'], 4],
      ['201', '201 signifie Created.', ['201 signifie interdit', '201 signifie mauvais body', '201 signifie non trouve'], 4],
      ['400', '400 signifie Bad Request, donnees invalides.', ['400 signifie succes', '400 signifie cree', '400 signifie timeout obligatoire'], 4],
      ['401', '401 signifie utilisateur non authentifie.', ['401 signifie interdit apres auth', '401 signifie cree', '401 signifie OK'], 4],
      ['403', '403 signifie authentifie mais pas autorise.', ['403 signifie manque de login', '403 signifie succes', '403 signifie JSON invalide'], 4],
      ['404', '404 signifie ressource introuvable.', ['404 signifie cree', '404 signifie suppression OK', '404 signifie app React'], 4],
      ['Postman', 'Postman permet de tester methodes, headers et body.', ['Postman remplace Flask', 'Postman ecrit le CSS', 'Postman interdit POST'], 5],
      ['cURL', 'cURL teste des requetes HTTP en ligne de commande.', ['cURL est une balise HTML', 'cURL est une table SQL', 'cURL est un hook React'], 5],
      ['CRUD', 'CRUD correspond a Create, Read, Update, Delete.', ['CRUD signifie CSS React UI Docker', 'CRUD est un status code', 'CRUD est une seule route'], 6]
    ].map(([label, correct, wrongs, ch]) => ({ label, correct, wrongs, ch, explain: correct }))
  }
];
...
for (const course of courses) {
  ...
  const bank = buildQuestions(course);
